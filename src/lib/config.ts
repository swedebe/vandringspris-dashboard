export type ConfigSource = "VITE" | "localStorage" | "missing";
...
export function getSupabaseConfig(): {
  ...
  // Partial availability: prefer complete LS over partial ENV
  if (!envUrl || !envKey) {
    if (lsUrl && lsKey) return { url: lsUrl, anonKey: lsKey, source: "localStorage" };
  }

  // Nothing complete
  return { url: envUrl || lsUrl, anonKey: envKey || lsKey, source: "missing" };
}
